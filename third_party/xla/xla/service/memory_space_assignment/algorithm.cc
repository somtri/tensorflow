...
absl::Status MsaAlgorithm::PinScalarBuffersInAlternateMemory(
    absl::Span<const HloBuffer* const> scalar_buffers_to_pin_in_alt_mem) {
  const auto& instruction_schedule = hlo_live_range_.instruction_schedule();
  for (const HloBuffer* buffer : scalar_buffers_to_pin_in_alt_mem) {
    RETURN_IF_ERROR(
        PinScalarBufferInAlternateMemory(buffer, instruction_schedule));
  }
  return absl::OkStatus();
}
...
absl::Status MsaAlgorithm::PinScalarBufferInAlternateMemory(
  ...
  //    (pinned) allocations and positions that get mirrored allocations. Note
  //    that due to scalar aliasing across tuple/slice instructions, a scalar
  //    buffer can span multiple non-trivial positions (HloPosition), each of
  //    which needs a pinned allocation during its live range.
  // 3. Find a chunk candidate for the buffer, skipping constant buffers or
  //    buffers that would OOM the alternate memory.
  // 4. Commit the chunk to the alternate memory using its actual live range.
  // 5. Create pinned allocations in alternate memory for the real positions
  //    using their exact live ranges (passing last_position_end_time = nullopt
  //    instead of artificially extending to program_end_time).
  ...
  for (const HloPosition& position : buffer->ComputePositions()) {
    if (IsTrivialPosition(position)) {
      TF_RET_CHECK(!position_to_live_range.contains(position))
  ...
  // so all real positions (starting at index 0) get pinned allocations.
  // Note: we pass last_position_end_time = std::nullopt (instead of extending
  // to program_end_time) so each position's exact true live range computed by
  // LiveRangeCalculator is used.
  TF_RET_CHECK(!non_trivial_positions_for_real_allocations.empty())
      << "Scalar buffer must have at least one real non-trivial position: "
      << buffer->ToString();
  CreatePinnedAllocationsInAltMemoryForPositions(
      non_trivial_positions_for_real_allocations, chunk_candidate,
      position_to_live_range, position_to_uses,
      /*async_conv_candidate_instructions=*/{}, allocations, colocations,
      /*last_position_end_time=*/std::nullopt);
...
absl::StatusOr<HeapSimulator::Result<HloValue>> MsaAlgorithm::Finish() {
  ...
  std::vector<const HloBuffer*> scalar_buffers_to_pin_in_alt_mem;
  for (MsaBufferInterval& interval : sorted_buffer_intervals) {
    HloInstruction* instruction = interval.buffer->instruction();
    const Shape& buffer_shape =
        ShapeUtil::GetSubshape(instruction->shape(), interval.buffer->index());
    // Check for unfinalized scalar buffers assigned to alternate memory.
    // Note: scalar buffers are excluded from block prefetching earlier in
    // GetBlockPrefetchedPositions, but if a scalar buffer was already
    // colored/pinned by an earlier pre-coloring pass, !IsValueFinalized ensures
    // we only process unfinalized scalar buffers here.
    if (ShapeUtil::IsEffectiveScalar(buffer_shape) &&
        buffer_shape.has_layout() &&
        buffer_shape.layout().memory_space() ==
            options_.alternate_memory_space &&
      ...
      scalar_buffers_to_pin_in_alt_mem.push_back(&scalar_buffer);
    }
  }

  absl::c_stable_sort(
      scalar_buffers_to_pin_in_alt_mem,
      [](const HloBuffer* a, const HloBuffer* b) { return a->id() < b->id(); });
  scalar_buffers_to_pin_in_alt_mem.erase(
      std::unique(scalar_buffers_to_pin_in_alt_mem.begin(),
